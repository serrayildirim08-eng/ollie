...
export function AppLockGate({
  ...
  const runBiometric = useCallback(async () => {
    ...
    try {
      ...
      // NOTE: 27ef92b's biometric.unlock accepts a second `purpose` arg
      // ('app' | 'finance-privacy'). main's signature still takes locale
      // only; drop the second arg here so AppLockGate compiles against
      // main. When the biometric.ts purpose-arg lands, restore `'app'`.
      const result = await impl('en');
      if (result.ok) {
        onUnlocked();
        return;
      }
