...
| B | **Encryption** (client-side AES-GCM-256 for body data before Supabase) | DEPRECATED-PIVOT | Pivot 2026-05-14 abandoned the server-blind default. `@ollie/crypto` retained for future opt-out path but no longer the default posture. See `project_ollie_b2b_pivot.md` + Sprint B'. | Replaced by consent + scrub + anonymized corpus pipeline; new posture is "opt-in, anonymize, off-switch any time" |
| I | **Consent reader** (`@ollie/consent` gates research pipeline + UI prompt) | WIRED-CROSSMODULE | `packages/consent/src/index.ts` ships `getConsent` / `setConsent` / `hasResearchConsent`; pre-pivot users force-null'd to re-prompt | Source of truth for opt-in. Sprint B' 2026-05-14. |
| J | **PII scrub** (`@ollie/pii-scrub`, locale-aware en/es/tr) | WIRED-CROSSMODULE | 3 layers (regex / numeric / wordlist) + 100-sample golden test (FP<5%, FN<10%) | All body brain-dump text passes through this before /label |
| K | **Research corpus + 10-sector cache** (`research_corpus` Supabase table + `@ollie/research-cache`) | WIRED-CROSSMODULE | Anonymized table (no user_id), 10 locked sectors, B2B-portal-aware indexes; `getSectorPatterns` exposes ≥10-count aggregates | New B2B asset. Body brain-dumps with body sector_hint feed this. |
