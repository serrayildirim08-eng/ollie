...
// ── C-model sleep-debt offer thresholds ──────────────────────────────────────
/** Minimum piled-up deficit (hours) before the "keep today lighter?" offer shows. */
const SLEEP_DEBT_OFFER_HOURS = 3;
/** Minimum logged nights in the window before the offer is trustworthy enough. */
const SLEEP_DEBT_MIN_NIGHTS = 5;

/** A sleep-debt offer pattern card, shaped for the brain gatherer (RawPattern). */
interface SleepDebtOfferCard {
  pattern: 'sleep-debt-high';
  signal: 'sleep_debt_offer';
  category: 'sleep-debt';
  copy: string;
  /** Synthetic single item so the existing gatherer carries the deficit hours
   *  in `days` (the copy layer's sleep_debt fallback reads facts.days). */
  items: Array<{ name: string; days: number }>;
  /** The action the brain surface attaches to this noticing. */
  actionKind: 'defer_tasks';
  /** Scores like an already-present concern (urgency curve treats now as due). */
  urgencyAt: number;
  ts: number;
...
/**
 * Build the sleep-debt → defer_tasks offer card, or null when the gate isn't
 * met (too little debt, too few nights). Pure — `debt` is the computeSleepDebt
 * result (totalDeficitHours / nightsCounted) and `now` is the wall clock.
 *
 * Copy is a calm ONE-sentence soft question with no streaks / shame; the final
 * user-facing sentence is regenerated by the copy layer (AI or the trilingual
 * sleep_debt fallback) — this string is the safe default the gatherer carries.
 */
function buildSleepDebtOffer(
  debt: { totalDeficitHours: number; nightsCounted: number },
  now: number,
): SleepDebtOfferCard | null {
  const hours = Math.round(debt.totalDeficitHours);
  if (debt.totalDeficitHours < SLEEP_DEBT_OFFER_HOURS) return null;
  if (debt.nightsCounted < SLEEP_DEBT_MIN_NIGHTS) return null;
  const confidence: 'medium' | 'high' = debt.nightsCounted >= 10 ? 'high' : 'medium';
  return {
    pattern: 'sleep-debt-high',
    signal: 'sleep_debt_offer',
    category: 'sleep-debt',
    copy: `about ${hours} hours of sleep debt have piled up — keep today lighter?`,
    items: [{ name: 'sleep', days: hours }],
    actionKind: 'defer_tasks',
    urgencyAt: now,
    ts: now,
    confidence,
    sample_n: debt.nightsCounted,
  };
}
...
export function createSleepOrchestrator(
  ...
  function recomputeDerived(): void {
    try {
      ...
      // ── C-model offer · sleep-debt → defer_tasks ──────────────────────
      // When a gentle amount of sleep debt has piled up (≥ SLEEP_DEBT_OFFER_HOURS
      // over ≥ SLEEP_DEBT_MIN_NIGHTS logged nights), surface ONE calm offer in
      // the cross-life brain surface: "keep today lighter?". Accepting it pushes
      // today's non-urgent work/admin tasks to tomorrow (native executeAction →
      // defer_tasks). Honours the show_sleep_debt UX flag. Emitted as a pattern
      // card so the existing brain gatherer picks it up with zero gatherer change
      // — the deficit hours ride in a single synthetic item's `days` slot, which
      // the copy layer's sleep_debt fallback reads. urgencyAt: now so it scores
      // like an already-present concern. NOTE: read the local `debt` variable
      // (totalDeficitHours), not the stale `debt.debt_hours` shape elsewhere.
      const sleepDebtOffer = settings.show_sleep_debt
        ? buildSleepDebtOffer(debt, now)
        : null;
      ...
      // ── B insight · caffeine_cutoff onset delay → facts.days ──────────────
      // The caffeine_cutoff detector (insight only, NO action) measures how much
      // late caffeine delays sleep onset. Carry that number into a synthetic
      // item's `days` slot so the brain copy layer's trilingual `caffeine`
      // fallback can name it ("…by about N minutes") — the detector's own copy is
      // TR-only. No actionKind is attached, so it stays insight-only.
      for (const p of patterns) {
        if (p?.pattern !== 'caffeine_cutoff') continue;
        const delta = (p as { onset_delta_min?: number | null }).onset_delta_min;
        if (typeof delta === 'number' && delta > 0) {
          (p as unknown as { items: Array<{ name: string; days: number }> }).items = [
            { name: 'caffeine', days: delta },
          ];
        }
      }
      ...
      // The sleep-debt offer is not a detector pattern (not in AnySleepPattern);
      // it carries an action for the brain surface. Append it to the written
      // slice so the gatherer (which reads tolerant pattern cards) sees it.
      const writtenPatterns = sleepDebtOffer
        ? [...patterns, sleepDebtOffer as unknown as AnySleepPattern]
        : patterns;

      setKey('patterns', writtenPatterns);
