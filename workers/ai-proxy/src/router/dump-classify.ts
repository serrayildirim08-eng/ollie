...
- If the user mentions a PRICE alongside the food item ("bought milk for $5", "got eggs $4.50"), still classify as grocery.pantry_add but include \`price\` (number) and \`currency\` (string, e.g. "USD"/"EUR") on the payload. The grocery handler will mirror the purchase to finance automatically — DO NOT emit a separate finance.log_transaction.
- Finance is only for explicit spending with no purchased item ("paid rent", "spent $40 on impulse stuff at sephora"), or for bills/transactions/subscriptions without a pantry implication.
